# OLLAMA

## Installation

```shell
brew install ollama
```

## Models

Choose the model which you prefer.

```shell
ollama pull alfred:latest
ollama pull all-minilm:latest
ollama pull bakllava:latest
ollama pull codebooga:latest
ollama pull codegemma:latest
ollama pull codellama:latest
ollama pull codeqwen:latest
ollama pull codeup:latest
ollama pull command-r:latest
ollama pull command-r-plus:latest
ollama pull dbrx:latest
ollama pull deepseek-coder:latest
ollama pull deepseek-llm:latest
ollama pull dolphin-llama3:latest
ollama pull dolphin-mistral:latest
ollama pull dolphin-mixtral:latest
ollama pull dolphin-phi:latest
ollama pull dolphincoder:latest
ollama pull duckdb-nsql:latest
ollama pull everythinglm:latest
ollama pull falcon:latest
ollama pull gemma:latest
ollama pull goliath:latest
ollama pull llama-pro:latest
ollama pull llama2:latest
ollama pull llama2-chinese:latest
ollama pull llama2-uncensored:latest
ollama pull llama3:latest
ollama pull llava:latest
ollama pull magicoder:latest
ollama pull meditron:latest
ollama pull medllama2:latest
ollama pull megadolphin:latest
ollama pull mistral:latest
ollama pull mistral-openorca:latest
ollama pull mistrallite:latest
ollama pull mixtral:latest
ollama pull mxbai-embed-large:latest
ollama pull neural-chat:latest
ollama pull nexusraven:latest
ollama pull nomic-embed-text:latest
ollama pull notus:latest
ollama pull notux:latest
ollama pull nous-hermes:latest
ollama pull nous-hermes2:latest
ollama pull nous-hermes2-mixtral:latest
ollama pull open-orca-platypus2:latest
ollama pull openchat:latest
ollama pull openhermes:latest
ollama pull orca-mini:latest
ollama pull orca2:latest
ollama pull phi:latest
ollama pull phi3:latest
ollama pull phind-codellama:latest
ollama pull qwen:latest
ollama pull samantha-mistral:latest
ollama pull snowflake-arctic-embed:latest
ollama pull solar:latest
ollama pull sqlcoder:latest
ollama pull stable-beluga:latest
ollama pull stable-code:latest
ollama pull stablelm-zephyr:latest
ollama pull stablelm2:latest
ollama pull starcoder:latest
ollama pull starcoder2:latest
ollama pull starling-lm:latest
ollama pull tinydolphin:latest
ollama pull tinyllama:latest
ollama pull vicuna:latest
ollama pull wizard-math:latest
ollama pull wizard-vicuna:latest
ollama pull wizard-vicuna-uncensored:latest
ollama pull wizardcoder:latest
ollama pull wizardlm:latest
ollama pull wizardlm-uncensored:latest
ollama pull wizardlm2:latest
ollama pull xwinlm:latest
ollama pull yarn-llama2:latest
ollama pull yarn-mistral:latest
ollama pull yi:latest
ollama pull zephyr:latest```
